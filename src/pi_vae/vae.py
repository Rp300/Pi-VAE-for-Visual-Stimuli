## This file implements all the code for VAE model.
...
def z_prior_nn(u_input, dim_lam):
    ...
    n_nodes = [max(dim_lam,dim_u), max(dim_lam,dim_u), dim_lam];
    act_func = ['tanh', 'tanh', 'linear'];
    n_layers = len(n_nodes);
    output = u_input;
...
## vae model
def vae_mdl(dim_x, dim_z, gen_nodes, n_blk=None, mdl='poisson', learning_rate=5e-4):
...
def simulate_data(length, n_cls, n_dim):
    ...
    z_true = np.hstack((z_true, 0.0001*np.random.normal(0,1,[z_true.shape[0],n_dim-2])));
