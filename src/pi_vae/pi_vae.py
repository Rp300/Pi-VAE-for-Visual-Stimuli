...
def pivae_mdl(dim_x, dim_z, dim_u, gen_nodes, n_blk=None, min_gen_nodes_decoder_nflow=30, mdl='poisson', disc=True, learning_rate=5e-4):
    """Define pi-vae model.
    ...
    if mdl == 'gaussian':
        ...
        vae = Model(inputs = [x_input, u_input, one_tensor], outputs = [post_mean, post_log_var, z_sample,fire_rate, lam_mean, lam_log_var, z_mean, z_log_var, obs_log_var], name='pivae')
    elif mdl == 'poisson':
        vae = Model(inputs = [x_input, u_input], outputs = [post_mean, post_log_var, z_sample, fire_rate, lam_mean, lam_log_var, z_mean, z_log_var], name='pivae')
