...
## sample from p(z|u)
def compute_marginal_lik_poisson(vae_mdl, y_test, u_fake, n_sample, log_opt=False):
    lik_all = [];
    for jj in range(len(y_test)): ## for each batch
        ...
        for ii in range(len(u_fake)): ## for each unique u value
            ...
            func_input = z_sample.reshape(-1,z_dim[-1])
            func_output = get_fire_rate_output([func_input])

            fire_rate = func_output[0];
            # print(f"1. y_test[jj] shape l: {y_test[jj].shape}, fire_rate shape: {fire_rate.shape}")
            fire_rate = fire_rate.reshape(n_sample, -1, fire_rate.shape[-1]);
            # print(f"2. y_test[jj] shape: {y_test[jj].shape}, fire_rate shape: {fire_rate.shape}")

            ## compute p(x|z) poisson likelihood ##
            loglik = y_test[jj]*np.log(np.clip(fire_rate, 1e-10, 1e7))-fire_rate;
